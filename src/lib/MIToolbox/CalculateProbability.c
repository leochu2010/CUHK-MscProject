...
JointProbabilityState calculateJointProbability(double *firstVector, double *secondVector, int firstVectorLength, int sencondVectorLength)
{
  ...
  int jointNumStates;
  int i;
  double length1 = firstVectorLength;
  double length2 = secondVectorLength;
  JointProbabilityState state;

  firstNormalisedVector = (int *) CALLOC_FUNC(firstVectorLength,sizeof(int));
  secondNormalisedVector = (int *) CALLOC_FUNC(secondVectorLength,sizeof(int));
  
  firstNumStates = normaliseArray(firstVector,firstNormalisedVector,firstVectorLength);
  secondNumStates = normaliseArray(secondVector,secondNormalisedVector,secondVectorLength);
  ...
  */
  ...
  {
    firstStateProbs[i] = firstStateCounts[i] / length1;
  }
  
  for (i = 0; i < secondNumStates; i++)
  {
    secondStateProbs[i] = secondStateCounts[i] / length2;
  }
  
  for (i = 0; i < jointNumStates; i++)
  {
    jointStateProbs[i] = jointStateCounts[i] / ((length1+length2)/2);
  }
